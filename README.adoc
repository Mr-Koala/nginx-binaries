...
// Download nginx with TCP/UDP proxy support (stream module)
await NginxBinary.download({ version: '1.28.x', variant: 'stream' })
// => '[...]/node_modules/.cache/nginx-binaries/nginx-1.28.0-stream-x86_64-linux'
...
.3+| _default_ +
stream footnote:stream[The `stream` variant includes TCP/UDP proxy modules for Layer 4 load balancing. See <<Stream Module Support>> for details.]
| Linux
| x86_64 +
aarch64 +
ppc64le +
...
The `stream` variant of nginx includes TCP/UDP proxy modules for Layer 4 load balancing and proxying. This enables nginx to act as a TCP/UDP load balancer in addition to its HTTP capabilities.

==== Included Stream Modules

*Core Stream Modules:*

* https://nginx.org/en/docs/stream/ngx_stream_core_module.html[ngx_stream_core_module] - Core stream functionality
* https://nginx.org/en/docs/stream/ngx_stream_proxy_module.html[ngx_stream_proxy_module] - TCP/UDP proxying
* https://nginx.org/en/docs/stream/ngx_stream_upstream_module.html[ngx_stream_upstream_module] - Upstream server groups

*Additional Stream Modules:*

* https://nginx.org/en/docs/stream/ngx_stream_ssl_module.html[ngx_stream_ssl_module] - SSL/TLS support for stream
* https://nginx.org/en/docs/stream/ngx_stream_realip_module.html[ngx_stream_realip_module] - Real IP address handling
* https://nginx.org/en/docs/stream/ngx_stream_ssl_preread_module.html[ngx_stream_ssl_preread_module] - SSL preread for SNI routing
...
[source, js, subs="+attributes"]
----
import { NginxBinary } from '{npm-name}'

// Download nginx with TCP/UDP proxy capabilities
...
See `examples/nginx-stream.conf` and `examples/download-stream-nginx.js` for complete configuration examples.
